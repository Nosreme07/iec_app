...
class EbdScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text("E.B.D", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
...
class _EbdSalasTabState extends State<EbdSalasTab> {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: const InputDecoration(labelText: "Nome da Sala", border: OutlineInputBorder()),
        ),
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              if (docId == null) {
                ...
                await FirebaseFirestore.instance.collection('ebd_salas').doc(docId).update({'nome': controller.text.trim()});
              }
  ...
  Future<void> _deleteSala(String docId, String nome) async {
    ...
    if (confirm) await FirebaseFirestore.instance.collection('ebd_salas').doc(docId).delete();
  }

  Future<void> _criarSalasPadrao() async {
    List<String> padroes = ["Mulheres", "Homens", "Crianças", "Adolescentes"];
    for (String nome in padroes) {
      await FirebaseFirestore.instance.collection('ebd_salas').add({'nome': nome, 'alunos': [], 'criado_em': FieldValue.serverTimestamp()});
    }
  }
        ...
        return Scaffold(
          ...
          floatingActionButton: canManage
            ? FloatingActionButton.extended(
                ...
                label: const Text("Nova Sala", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
              ) : null,
          body: StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance.collection('ebd_salas').orderBy('nome').snapshots(),
                ...
                return Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text("Nenhuma sala cadastrada.", style: TextStyle(fontSize: 16, color: Colors.grey[600])),
                      const SizedBox(height: 20),
                      if (canManage)
                        ElevatedButton.icon(
                          onPressed: _criarSalasPadrao, icon: const Icon(Icons.auto_awesome), label: const Text("Criar Salas Padrão"),
                          style: ElevatedButton.styleFrom(backgroundColor: ebdColor, foregroundColor: Colors.white),
                        )
                    ],
                  ),
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      leading: CircleAvatar(backgroundColor: ebdColor.withOpacity(0.2), child: const Icon(Icons.meeting_room, color: ebdColor)),
                      title: Text(nome, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                      subtitle: Text("${alunos.length} alunos matriculados", style: TextStyle(color: Colors.grey[600])),
                      trailing: canManage 
                        ? Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              IconButton(icon: const Icon(Icons.edit, color: Colors.blue), onPressed: () => _showSalaDialog(docId: docId, nomeAtual: nome)),
                              IconButton(icon: const Icon(Icons.delete, color: Colors.red), onPressed: () => _deleteSala(docId, nome)),
                            ],
                          )
                        : const Icon(Icons.chevron_right),
                      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => EbdChamadaScreen(salaId: docId, salaNome: nome))),
                    ),
                  );
                },
              );
...
class _EbdRelatoriosTabState extends State<EbdRelatoriosTab> {
  ...
  void initState() {
    ...
    _dataSelecionada = DateTime(_dataSelecionada.year, _dataSelecionada.month, _dataSelecionada.day);
  }
  ...
  Widget build(BuildContext context) {
    ...
    String textoFiltro = _isMonthly 
      ? DateFormat('MMMM yyyy', 'pt_BR').format(_dataSelecionada).toUpperCase()
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    Switch(value: _isMonthly, activeColor: ebdColor, onChanged: (val) => setState(() => _isMonthly = val)),
                    const Text("Mensal", style: TextStyle(fontWeight: FontWeight.bold)),
                  ],
                ),
                Row(
                  ...
                  children: [
                    ...
                    Text(textoFiltro, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: ebdColor)),
            ...
            child: StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapshotSalas) {
                ...
                return StreamBuilder<QuerySnapshot>(
                  stream: FirebaseFirestore.instance.collection('ebd_registros')
                  ...
                  builder: (context, snapshotRegistros) {
                    ...
                    int qtdAulas = diasLetivos.length;
                    double mediaPresentes = qtdAulas > 0 ? totalPresentes / qtdAulas : 0;
                    double mediaAusentes = qtdAulas > 0 ? totalAusentes / qtdAulas : 0;
                      ...
                      children: [
                          ...
                          children: [
                            Card(
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  children: [
                                    const Text("MÉDIA DE FREQUÊNCIA DA E.B.D", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                                    ...
                                    if (totalMatriculadosGlobal == 0)
                                      ...
                                      ClipRRect(
                                        ...
                                        child: Row(
                                          children: [
                                            Expanded(flex: totalPresentes, child: Container(height: 20, color: Colors.green)),
                                            Expanded(flex: totalAusentes, child: Container(height: 20, color: Colors.red)),
                                          ],
                                        ),
                                      ),
                                      ...
                                      Row(
                                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                        children: [
                                          Text("Presenças: ${percGeral.toStringAsFixed(1)}%", style: const TextStyle(color: Colors.green, fontWeight: FontWeight.bold)),
                                          Text("Faltas: ${(100-percGeral).toStringAsFixed(1)}%", style: const TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
                                        ],
                                      ),
                                      ...
                                      Row(
                                        ...
                                        children: [
                                          _buildMediaItem(Icons.people, "Média Pres.", mediaPresentes.toStringAsFixed(1)),
                                          _buildMediaItem(Icons.person_off, "Média Faltas", mediaAusentes.toStringAsFixed(1)),
                            ...
                            const SizedBox(height: 16),
                            const Text("Salas e Alunos Matriculados:", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.black87)),
                            const SizedBox(height: 8),
                            ...
                            ...dadosPorSala.entries.map((entry) {
                              ...
                              return Card(
                                ...
                                child: ExpansionTile(
                                  ...
                                  subtitle: Text("${alunosDaSala.length} alunos | $aulas aulas", style: const TextStyle(fontSize: 12)),
                                  children: [
                                    ...
                                    ...alunosDaSala.map((nomeAluno) {
                                      return ListTile(
                                        ...
                                        trailing: const Icon(Icons.analytics, color: Colors.blue, size: 20),
                        ...
                        Positioned(
                          ...
                          child: FloatingActionButton.extended(
                            ...
                            onPressed: () => EbdStatsHelper.opcoesGerarPdf(context, salasDocs),
                          ),
                        )
                      ],
                    );
...
class _EbdAlunosTabState extends State<EbdAlunosTab> {
  ...
  void _addAlunoGlobalDialog(List<QueryDocumentSnapshot> salas) {
    if (salas.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Crie uma sala primeiro!")));
      return;
    }
        ...
        builder: (context, setStateDialog) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              DropdownButtonFormField<String>(
                ...
                decoration: const InputDecoration(labelText: "Selecione a Turma", border: OutlineInputBorder()),
                items: salas.map((s) => DropdownMenuItem(value: s.id, child: Text(s['nome']))).toList(),
                onChanged: (val) => setStateDialog(() => selectedSalaId = val),
              ),
              const SizedBox(height: 15),
              TextField(
          ...
          actions: [
            ...
            ElevatedButton(
              style: ElevatedButton.styleFrom(backgroundColor: ebdColor, foregroundColor: Colors.white),
              onPressed: () async {
                if (controller.text.trim().isEmpty || selectedSalaId == null) return;
                await FirebaseFirestore.instance.collection('ebd_salas').doc(selectedSalaId).update({
      ...
      builder: (context, snapshot) {
        ...
        for (var sala in salas) {
          ...
          for (var aluno in alunosDaSala) {
            todosAlunos.add({'nome': aluno, 'sala': salaNome, 'salaId': salaId});
          }
        }
        todosAlunos.sort((a, b) => a['nome'].toLowerCase().compareTo(b['nome'].toLowerCase()));
        ...
        return Scaffold(
          ...
          body: Column(
            children: [
              Container(
                padding: const EdgeInsets.all(12), color: ebdColor.withOpacity(0.1),
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text("Total: ${todosAlunos.length} Matriculados", style: const TextStyle(fontWeight: FontWeight.bold, color: ebdColor)),
                    ElevatedButton.icon(
                      onPressed: () => EbdStatsHelper.opcoesGerarPdf(context, salas), 
                      icon: const Icon(Icons.picture_as_pdf, size: 16), label: const Text("PDF"),
                      style: ElevatedButton.styleFrom(backgroundColor: Colors.red[700], foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 10)),
                    )
                  ],
                ),
              ),
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        leading: const CircleAvatar(backgroundColor: ebdColor, child: Icon(Icons.person, color: Colors.white)),
                        ...
                        onTap: () => EbdStatsHelper.showOptions(context, aluno['nome'], aluno['salaId']),
                      ),
                    );
                  }
                ),
          ...
          floatingActionButton: FloatingActionButton.extended(
            ...
            label: const Text("Novo Aluno", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
          ),
        );
      }
    );
...
class EbdChamadaScreen extends StatefulWidget {
  ...
  const EbdChamadaScreen({super.key, required this.salaId, required this.salaNome});
...
class _EbdChamadaScreenState extends State<EbdChamadaScreen> {
  ...
  void _addAlunoDialog() {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: TextField(controller: controller, textCapitalization: TextCapitalization.words, decoration: const InputDecoration(labelText: "Nome Completo", border: OutlineInputBorder())),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text("Cancelar")),
          ElevatedButton(
            style: ElevatedButton.styleFrom(backgroundColor: ebdColor, foregroundColor: Colors.white),
            onPressed: () async {
              if (controller.text.trim().isEmpty) return;
              await FirebaseFirestore.instance.collection('ebd_salas').doc(widget.salaId).update({'alunos': FieldValue.arrayUnion([controller.text.trim()])});
              if (mounted) Navigator.pop(context);
            },
            child: const Text("Adicionar"),
          )
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              if (controller.text.trim().isEmpty || controller.text.trim() == nomeAntigo) return;
              await FirebaseFirestore.instance.collection('ebd_salas').doc(widget.salaId).update({'alunos': FieldValue.arrayRemove([nomeAntigo])});
              await FirebaseFirestore.instance.collection('ebd_salas').doc(widget.salaId).update({'alunos': FieldValue.arrayUnion([controller.text.trim()])});
              if (mounted) Navigator.pop(context);
            },
            child: const Text("Salvar"),
  ...
  void _removerAluno(String nomeAluno) async {
    ...
    if (confirm) await FirebaseFirestore.instance.collection('ebd_salas').doc(widget.salaId).update({'alunos': FieldValue.arrayRemove([nomeAluno])});
  }

  Future<void> _salvarDados(List<String> listaTotal, {required bool isEncerrar}) async {
    setState(() => _isSaving = true);
    try {
      List<String> presentes = [];
      for (String aluno in listaTotal) { if (_presencas[aluno] == true) presentes.add(aluno); }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Colors.grey[100],
      appBar: AppBar(
        title: Text("Sala: ${widget.salaNome}", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
        backgroundColor: ebdColor, iconTheme: const IconThemeData(color: Colors.white),
        actions: _aulaIniciada ? [IconButton(icon: const Icon(Icons.person_add), tooltip: "Adicionar Aluno", onPressed: _addAlunoDialog)] : null,
      ),
      body: StreamBuilder<DocumentSnapshot>(
        ...
        builder: (context, snapshot) {
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 20),
                        TextField(
                          controller: _professorController, textCapitalization: TextCapitalization.words,
          ...
          return Column(
            children: [
              Container(
                color: ebdColor.withOpacity(0.1), padding: const EdgeInsets.all(12),
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text("Prof: ${_professorController.text}", style: const TextStyle(fontWeight: FontWeight.bold, color: ebdColor)),
                    Text("Matriculados: ${alunos.length}", style: const TextStyle(fontWeight: FontWeight.bold, color: ebdColor)),
                  ],
                ),
              ),
              ...
              Expanded(
                child: alunos.isEmpty 
                  ? Center(child: Text("Nenhum aluno.\nClique no + lá em cima.", textAlign: TextAlign.center, style: TextStyle(color: Colors.grey[600])))
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Container(
                          ...
                          child: CheckboxListTile(
                            title: Text(aluno, style: const TextStyle(fontWeight: FontWeight.w500)),
                            value: _presencas[aluno], activeColor: Colors.green, checkColor: Colors.white,
                            secondary: Row(
                              mainAxisSize: MainAxisSize.min,
                              children: [
                                IconButton(icon: const Icon(Icons.edit, color: Colors.blue, size: 20), onPressed: () => _editAlunoDialog(aluno)),
                                IconButton(icon: const Icon(Icons.delete_outline, color: Colors.red, size: 20), onPressed: () => _removerAluno(aluno)),
                              ],
                            ),
                            onChanged: (bool? value) { setState(() { _presencas[aluno] = value ?? false; }); },
                          ),
                        );
                      },
                    ),
              ...
              Container(
                ...
                child: Column(
                  children: [
                    TextField(
                      controller: _ofertaController, keyboardType: const TextInputType.numberWithOptions(decimal: true),
                      decoration: InputDecoration(
                        labelText: "Valor Arrecadado (Oferta)", prefixIcon: const Icon(Icons.attach_money, color: Colors.green),
                        hintText: "0.00", border: OutlineInputBorder(borderRadius: BorderRadius.circular(10)),
                      ...
                      children: [
                        Expanded(
                          child: ElevatedButton.icon(
                            onPressed: _isSaving ? null : () => _salvarDados(alunos, isEncerrar: false),
                        ...
                        Expanded(
                          child: ElevatedButton.icon(
                            onPressed: _isSaving ? null : () => _salvarDados(alunos, isEncerrar: true),
...
class EbdStatsHelper {
  ...
  static void abrirSeletorData(BuildContext context, {required String tipo, required Function(int ano, int? mes, int? dia) onConfirm}) async {
    if (tipo == 'diario') {
      ...
      showDialog(
        ...
        builder: (ctx) => StatefulBuilder(
          builder: (context, setStateDialog) => AlertDialog(
            title: Text(tipo == 'anual' ? "Selecione o Ano" : "Selecione Mês e Ano", style: const TextStyle(color: ebdColor, fontWeight: FontWeight.bold)),
            content: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                if (tipo == 'mensal') ...[
                  DropdownButtonFormField<int>(
                    ...
                    items: List.generate(12, (index) => DropdownMenuItem(value: index + 1, child: Text(meses[index]))),
                ...
                DropdownButtonFormField<int>(
                  ...
                  items: List.generate(10, (index) => DropdownMenuItem(value: DateTime.now().year - 5 + index, child: Text("${DateTime.now().year - 5 + index}"))),
            ...
            actions: [
              ...
              ElevatedButton(
                ...
                onPressed: () {
                  Navigator.pop(ctx);
                  onConfirm(selectedYear, tipo == 'mensal' ? selectedMonth : null, null);
                },
  ...
  static void showOptions(BuildContext context, String nomeAluno, String salaId) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text(nomeAluno, style: const TextStyle(fontWeight: FontWeight.bold, color: ebdColor)),
        ...
        actions: [
          ElevatedButton(
            ...
            onPressed: () {
              Navigator.pop(ctx);
              abrirSeletorData(context, tipo: 'mensal', onConfirm: (ano, mes, dia) {
                _mostrarDetalhesMensal(context, nomeAluno, salaId, ano, mes!);
              });
            },
            child: const Text("Mensal", style: TextStyle(color: Colors.white)),
          ),
          ElevatedButton(
            style: ElevatedButton.styleFrom(backgroundColor: Colors.indigo),
            onPressed: () {
              Navigator.pop(ctx);
              abrirSeletorData(context, tipo: 'anual', onConfirm: (ano, mes, dia) {
                _mostrarDetalhesAnual(context, nomeAluno, salaId, ano);
              });
            },
            child: const Text("Anual", style: TextStyle(color: Colors.white)),
          ),
        ],
      )
    );
  ...
  static void _mostrarDetalhesMensal(BuildContext context, String nomeAluno, String salaId, int ano, int mes) async {
    ...
    try {
      var registros = await FirebaseFirestore.instance.collection('ebd_registros')
      ...
      showDialog(
        context: context,
        builder: (ctx) => AlertDialog(
          title: Text("$nomeAluno - ${DateFormat('MMMM yyyy', 'pt_BR').format(inicioBusca)}", style: const TextStyle(fontWeight: FontWeight.bold)),
          content: SizedBox(
            ...
            child: dias.isEmpty 
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (c, i) {
                    ...
                    return ListTile(
                      leading: Icon(Icons.calendar_today, size: 20, color: Colors.grey[600]),
                      title: Text("Dia ${DateFormat('dd/MM/yyyy').format(d)}"),
                      trailing: Text(pres ? "Presente" : "Falta", style: TextStyle(color: pres ? Colors.green : Colors.red, fontWeight: FontWeight.bold)),
                    );
                  }
                ),
          ),
      ...
      if (context.mounted) { Navigator.pop(context); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Erro: $e"))); }
    }
  }

  static void _mostrarDetalhesAnual(BuildContext context, String nomeAluno, String salaId, int ano) async {
    showDialog(context: context, builder: (c) => const Center(child: CircularProgressIndicator()));
    
    DateTime inicioBusca = DateTime(ano, 1, 1);
    DateTime fimBusca = DateTime(ano, 12, 31, 23, 59, 59);
    List<String> nomeMeses = ["Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"];
    ...
    try {
      ...
      List<Map<String, int>> statsMeses = List.generate(12, (_) => {'p': 0, 'f': 0, 'aulas': 0});
      ...
      showDialog(
        context: context,
        builder: (ctx) => AlertDialog(
          title: Text("$nomeAluno - Frequência $ano", style: const TextStyle(fontWeight: FontWeight.bold)),
          content: SizedBox(
            width: double.maxFinite,
            child: ListView.builder(
              shrinkWrap: true,
              ...
              itemBuilder: (c, i) {
                ...
                return ListTile(
                  title: Text(nomeMeses[i], style: const TextStyle(fontWeight: FontWeight.bold)),
                  subtitle: Text("${s['p']} presentes e ${s['f']} faltas"),
                  leading: const Icon(Icons.assessment, color: ebdColor),
                );
              }
            ),
          ),
  ...
  static void opcoesGerarPdf(BuildContext context, List<DocumentSnapshot> salasDocs) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text("Exportar Relatório PDF", style: TextStyle(color: ebdColor, fontWeight: FontWeight.bold)),
        ...
        actions: [
          ElevatedButton(
            ...
            onPressed: () {
              Navigator.pop(ctx);
              abrirSeletorData(context, tipo: 'diario', onConfirm: (ano, mes, dia) {
                _buscarRegistrosEPdf(context, salasDocs, 'diario', ano, mes, dia);
              });
            },
            child: const Text("Diário", style: TextStyle(color: Colors.white)),
          ),
          ElevatedButton(
            style: ElevatedButton.styleFrom(backgroundColor: ebdColor),
            onPressed: () {
              Navigator.pop(ctx);
              abrirSeletorData(context, tipo: 'mensal', onConfirm: (ano, mes, dia) {
                _buscarRegistrosEPdf(context, salasDocs, 'mensal', ano, mes, null);
              });
            },
            child: const Text("Mensal", style: TextStyle(color: Colors.white)),
          ),
          ElevatedButton(
            ...
            onPressed: () {
              Navigator.pop(ctx);
              abrirSeletorData(context, tipo: 'anual', onConfirm: (ano, mes, dia) {
                _buscarRegistrosEPdf(context, salasDocs, 'anual', ano, null, null);
              });
            },
            child: const Text("Anual", style: TextStyle(color: Colors.white)),
          ),
  ...
  static void _buscarRegistrosEPdf(BuildContext context, List<DocumentSnapshot> salasDocs, String tipo, int ano, int? mes, int? dia) async {
    showDialog(context: context, builder: (c) => const Center(child: CircularProgressIndicator()));
    ...
    try {
      ...
      if (context.mounted) Navigator.pop(context);
      PdfEbdGenerator.gerarPdf(context, tipo: tipo, dataRef: dataRef, salas: salasDocs, registros: registros.docs);
    } catch(e) {
      if (context.mounted) { Navigator.pop(context); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Erro: $e"))); }
    }
  }
...
class PdfEbdGenerator {
  static Future<void> gerarPdf(BuildContext context, {required String tipo, required DateTime dataRef, required List<DocumentSnapshot> salas, required List<DocumentSnapshot> registros}) async {
    ...
    if (tipo == 'diario') {
      ...
      periodoTexto = "${DateFormat('MMMM', 'pt_BR').format(dataRef).toUpperCase()} de ${dataRef.year}";
    } else {
    ...
    for (var s in salas) {
      ...
      for (var aluno in List<String>.from(s['alunos'] ?? [])) {
        estatisticasPorSala[sId]!['alunos'][aluno] = {'presencas': 0, 'faltas': 0};
      }
    }
    ...
    for (var r in registros) {
      ...
      totalFaltasGeral += a;
      totalOfertasGeral += o;
      diasLetivosGeral.add(data['data_str']);
    ...
    int qtdDomingos = diasLetivosGeral.length;
    double mediaPresentes = qtdDomingos > 0 ? totalPresentesGeral / qtdDomingos : 0;
    double mediaFaltas = qtdDomingos > 0 ? totalFaltasGeral / qtdDomingos : 0;
    int totalMatriculados = totalPresentesGeral + totalFaltasGeral;
    double percGeral = totalMatriculados > 0 ? (totalPresentesGeral / totalMatriculados) * 100 : 0;
      ...
      pw.MultiPage(
        ...
        footer: (pw.Context context) {
          return pw.Container(
            ...
            child: pw.Text("Gerado pelo aplicativo IECM em $dataAtual", style: const pw.TextStyle(color: PdfColors.grey, fontSize: 10)),
          );
        },
        build: (pw.Context context) {
          return [
            // CABEÇALHO OFICIAL EM TEXTO
            pw.Center(child: pw.Text("Igreja Evangélica Congregacional em Moreno", style: pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold))),
            pw.Center(child: pw.Text("Rua Luiz Cavalcante Lins, 353. Alto da Liberada - Moreno/PE", style: const pw.TextStyle(fontSize: 10))),
            pw.Center(child: pw.Text("CNPJ: 30.057.670.0001-05", style: const pw.TextStyle(fontSize: 10))),
            pw.SizedBox(height: 20),
            
            pw.Center(child: pw.Text("Relatório Escola Bíblica Dominical - EBD ($periodoTexto)", style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold))),
            pw.SizedBox(height: 20),

            // RESUMO GERAL COM MÉDIAS
            pw.Text("RESUMO GERAL", style: pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold)),
            pw.Divider(),
            pw.Text("Média de Presentes: ${mediaPresentes.toStringAsFixed(1)}"),
            pw.Text("Média de Faltas: ${mediaFaltas.toStringAsFixed(1)}"),
            pw.Text("Frequência Geral Média: ${percGeral.toStringAsFixed(1)}%"),
            pw.Text("Arrecadação Total: R\$ ${totalOfertasGeral.toStringAsFixed(2)}"),
            pw.SizedBox(height: 20),
            ...
            ...estatisticasPorSala.entries.map((entry) {
              ...
              sData['alunos'].forEach((alunoNome, stats) {
                ...
                alunosWidgets.add(
                  pw.Padding(
                    ...
                    child: pw.Text("- $alunoNome: $p presenças | $f faltas (${percAluno.toStringAsFixed(1)}%)", style: const pw.TextStyle(fontSize: 12))
                  )
                );
              });
              ...
              return pw.Column(
                ...
                children: [
                  ...
                  if (alunosWidgets.isEmpty) pw.Text("Nenhum aluno matriculado.", style: pw.TextStyle(fontStyle: pw.FontStyle.italic, fontSize: 12)),
                  ...alunosWidgets,
                  pw.SizedBox(height: 20),
                ]
              );
    ...
    await Printing.sharePdf(bytes: await pdf.save(), filename: 'EBD_Relatorio_$periodoTexto.pdf');
  }
}